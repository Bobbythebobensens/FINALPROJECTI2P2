...
LAST CHANGES =================================================================
What Changed: Added Alpha-Beta Pruning to the Search Engine

The original engine used a basic minimax/negamax search that evaluated every possible move sequence up to the selected depth. While this works correctly, it spends a lot of time analyzing moves that are clearly worse than options it has already found.

For example, if the engine has already found a move that wins a Queen, there's little value in fully analyzing another move that immediately loses a Queen. The original search would still spend time exploring that losing line.

To solve this, I implemented Alpha-Beta Pruning, a common optimization used in chess engines.

How It Works

Alpha-Beta Pruning keeps track of two values while searching:

Alpha = the best score the current player has found so far.
Beta = the best score the opponent can force.

As moves are evaluated, these values are updated. If the engine discovers that a position cannot possibly produce a better result than one it has already found, it immediately stops searching that branch.

In other words, instead of examining every possible continuation, the engine skips branches that a strong opponent would never allow to happen.

Changes Made
1. Added Alpha and Beta Parameters

The recursive search function was updated to receive alpha and beta values, allowing score bounds to be passed down through the search tree.

2. Initialized Bounds at the Root

At the start of the search:

Alpha is set to negative infinity.
Beta is set to positive infinity.

As better moves are discovered, alpha is increased accordingly.

3. Updated Recursive Calls

Because the engine uses a Negamax framework, the alpha and beta values must be flipped and negated when searching the opponent's turn. This ensures the search remains mathematically correct while alternating perspectives between players.

4. Added Pruning Logic

After evaluating each move:

The best score found so far is used to update alpha.
If alpha becomes greater than or equal to beta, the remaining moves are skipped.

This is known as an alpha-beta cutoff.

At that point, the engine has already proven that the current branch cannot improve the final decision, so continuing the search would only waste time.

Result

The engine now reaches the same decisions while examining far fewer positions. This significantly improves search speed and allows deeper searches within the same amount of time.

In short:

Before: Search every branch, even obviously bad ones.
After: Skip branches that cannot affect the final decision.
This makes the engine much more efficient without changing the quality of its evaluations.

how it works and what code changed:


1. The Core Recursive Function: eval_ctx
This function is the heart of the engine. It takes a board state, the remaining depth to search, and the alpha and beta bounds.

Part A: Base Cases and Terminal States
Before exploring any moves, the engine checks if the game is already over or if it has hit the depth limit.

cpp
// 1. Ensure we have generated the legal moves for this state
if(state->legal_actions.empty() && state->game_state == UNKNOWN){
    state->get_legal_actions();
}
// 2. Check for checkmate (King capture in MiniChess)
if(state->game_state == WIN){
    return P_MAX - ply; // Prefer faster wins by subtracting the ply count!
}
// 3. Check for natural draws
if(state->game_state == DRAW){
    return 0;
}
// 4. Check for 4-fold repetition (game history)
int rep_score;
if(state->check_repetition(history, rep_score)){
    return rep_score;
}
history.push(state->hash());
// 5. Depth Limit Reached
if(depth <= 0){
...
It first forces move generation if it hasn't happened yet.
If the current player has won, it returns P_MAX (a huge positive number) minus the current ply (how deep we are). Subtracting the ply is a clever trick: it teaches the AI that a mate in 2 is mathematically better than a mate in 6, forcing it to play aggressively.
If it's a draw, it returns 0 (neutral).
If depth hits 0, the engine stops searching and calls the static evaluate() function to roughly guess who is winning based on piece values and board position.
Part B: The Alpha-Beta Loop
If the game isn't over, the engine starts trying out every possible legal move.

cpp
int best_score = M_MAX; // Start by assuming the worst possible score (-infinity)
for(auto& action : state->legal_actions){
    // 1. Play the move on a new board
    State* next = static_cast<State*>(state->next_state(action));
    next->get_legal_actions();
    bool same = next->same_player_as_parent();
    // 2. Recursively evaluate the child board
    int score;
    if (same) {
        // Very rare (e.g. extra turns). Bounds stay the same.
        score = eval_ctx(next, depth, alpha, beta, history, ply + 1, ctx, p);
    } else {
        // Normal chess move. Turn passes to opponent.
        // We drop depth by 1, and FLIP the alpha/beta bounds!
        score = -eval_ctx(next, depth - 1, -beta, -alpha, history, ply + 1, ctx, p);
    }
    delete next; // Free memory to prevent leaks!
    // 3. Keep track of the best move seen so far
    if(score > best_score){
        best_score = score;
    }
    // 4. Update Alpha (our guaranteed minimum)
    if(best_score > alpha){
        alpha = best_score;
    }
    // 5. The Pruning Cutoff!
    if(alpha >= beta){
        break; 
    }
}
...
state->next_state(action) creates a brand new board with the move played.
It calls eval_ctx on that new board. Notice the -eval_ctx(..., -beta, -alpha)? Because it's the opponent's turn on the next board, what is good for them is bad for us. We negate the score to bring it back to our perspective. We also swap and negate alpha and beta because our floor becomes their ceiling.
Finally, if alpha >= beta, we hit our cutoff. We break out of the loop and skip creating/evaluating any remaining moves in legal_actions, which is where the massive speed boost comes from.
2. The Root Function: search
The search function is what the UBGI protocol (or the CLI/GUI) actually calls to start the engine.
...
SearchResult MiniMax::search(...) {
    ...
    for(auto& action : state->legal_actions){
        ...
        if(score > best_score){
            best_score = score;
            result.best_move = action; // Save the actual move!
            // Tell the GUI we found a new best move (Partial PV updates)
        ...
        if(best_score > alpha){
            alpha = best_score; // Narrow the window for the next root moves
        }
        move_index++;
    }
...
What it does differently than eval_ctx:

It remembers the move: eval_ctx only cares about the mathematical score of a position. The search function actually needs to remember which specific action (e.g., e2e4) resulted in that score so it can tell the GUI what move to play on the board.
It communicates with the GUI: Every time it finds a move that beats the current best_score, it fires ctx.on_root_update. This is what makes the engine's "current best line" (PV) update live on the screen while it's thinking.
No Cutoffs at the Root: Notice there is no if (alpha >= beta) break; in the root search. We always want to at least look at every single legal move from the starting position to ensure we find the absolute best one, we just use the alpha value to search the later moves much faster.
